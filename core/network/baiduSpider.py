...
class Crawler:
    ...
    def save_image(self, rsp_data, word):
        ...
        for image_info in rsp_data['data']:
            try:
                ...
                print("picture+1,已有" + str(self.__counter) + "张picture")
                self.__counter += 1
        return
    ...
    def start(self, word, total_page=1, start_page=1, per_page=30):
        ...
        # __start_amount为当前页图片总数,等于页码*每页数量,也即单次训练数量
...
if __name__ == '__main__':
    # 此处可通过命令行+指定参数运行,运行方式如下
    # python baiduSpider.py -w 'word' -tp 30 sp 1 -pp 100 -d 0.05
    if len(sys.argv) > 1:
        ...
        # 抓取关键词为 “word”，总数为 1 页（即总共 1*60=60 张），开始页码为 2
